...
export const GET = async () => {
  ...
  const { data, error } = await t(db, 'poll_proposals')
    .select('id, question, region, options, source_of_truth, resolution_criteria, cutoff_at, resolves_at, status, moderator_notes, poll_id, created_at, moderated_at, topics(slug, label), profiles(handle)')
